...
export default defineConfig(({ mode }) => {
  ...
  const project = env.VITE_GCP_PROJECT || 'deft-orb-494315-v4';
  const location = env.VITE_GCP_LOCATION || 'us-central1';
            ...
            if (req.url.startsWith('/api/openrouter') && req.method === 'POST') {
              ...
              req.on('end', async () => {
                try {
                  ...
                  if (!openRouterApiKey) {
                    res.statusCode = 401;
                    res.end(JSON.stringify({ error: 'OpenRouter API 키가 설정되지 않았습니다. .env 파일에 OPENROUTER_API_KEY를 추가해주세요.' }));
                    return;
                  }
                  ...
                  const orRes = await fetch('https://openrouter.ai/api/v1/chat/completions', {
                    ...
                    headers: {
                      ...
                      'HTTP-Referer': 'https://prompt-hub.app',
                      'X-Title': 'PROMPT.HUB',
                    },
